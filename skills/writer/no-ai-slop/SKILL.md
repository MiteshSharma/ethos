...
version: 1.0.0
# Third-party skill, vendored verbatim. The body below and `eval.md` are the
# upstream author's work — only this frontmatter block was adapted to the Ethos
# skill contract. Source: https://github.com/petergyang/no-ai-slop
author: petergyang — https://github.com/petergyang/no-ai-slop
tags: [writer, writing, editing, voice, ai-slop]
# Deliberately empty: this skill only reads and rewrites prose that is already
# in the conversation, so it needs no tool. In `capability` ingest mode an empty
# declaration is included by default (`fallback_unknown: allow`), which is what
# makes it reachable from every personality regardless of toolset.
required_tools: []
...
ethos:
  ...
  prerequisites:
    ...
    auth: []
    env_vars: []
    optional_tools: [read_file, write_file, patch_file]
  integrates_with: []
  surface_metadata:
    invocation_trigger: "user says 'make this less AI-sounding', 'edit this draft', 'tighten this up', 'remove the AI slop', 'does this read as AI?', 'audit this for AI patterns'"
    estimated_turns: "1-3"
---
