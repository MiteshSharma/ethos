...
`runSubAgent` (`src/index.ts:25`) drives `loop.run(prompt, ...)` to completion, accumulating `text_delta` events into a single string and breaking on `done`. `delegate_task` derives a child session key as `${ctx.sessionKey}:sub:${label}:${personalityId}:${currentTurn}` — the resolved child personality is part of the key because a session's personality is bound at creation, so a same-turn fan-out to several personalities needs one session each (the personality segment is empty when no personality is in play); `mixture_of_agents` uses `:moa:${label}:${currentTurn}` per agent and `:moa:synthesis:${currentTurn}` for the optional synthesis pass (`src/index.ts:201,241`). `Promise.allSettled` is used so one slow or failing agent doesn't block the rest, and a result with zero successes returns an error.
