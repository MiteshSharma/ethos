...
packages/wiring/src/backup-schedule.ts mkdirSync/writeFileSync/readFileSync/statSync/unlinkSync — the scheduled backup's `backups/.lock` sentinel: an advisory `wx`-flag exclusive create, held for the duration of a run so a manual `ethos backup` and the cron job cannot stream the same databases into two archives at once, with stale detection by the holder's pid and boot (`backup/holder-identity.ts`) and by mtime when the body carries no readable pid. The same primitive and the same carve-out as `extensions/agent-mesh/src/index.ts`'s registry lock and `restore.ts`'s `.restore-in-progress` sentinel — `exists()` then `write()` is the exact race the lock exists to close, and no `Storage` interface can express an atomic create-if-absent. Rotation, which deletes archives, deliberately does NOT take this carve-out: it goes through `Storage.listEntries`/`remove` like ordinary code
packages/wiring/src/backup/holder-identity.ts readFileSync — reads `/proc/sys/kernel/random/boot_id` on Linux, the kernel's per-boot UUID, so the `.restore-in-progress` sentinel and the `backups/.lock` body can record WHICH BOOT their pid belongs to. A system path under `/proc`, never `~/.ethos/` — the same rationale as `extensions/skills/src/skill-compat.ts`'s `$PATH` walk. It exists because `process.kill(pid, 0)` answers "is some process wearing this number", not "is the holder still running": after a reboot a recycled pid reads as alive and the lock would never expire. That used to be capped with a wall clock (24h/36h), which preempted holders that were demonstrably ALIVE — a silent two-writer corruption traded against a deadlock that is loud and an operator can clear. Boot identity answers the recycled-pid case exactly instead. macOS derives `kern.boottime` from `os.uptime()` without spawning `sysctl`; every other platform records nothing, and an unknown or mismatched boot identity means "cannot prove a different boot", so a live pid is never taken over automatically
