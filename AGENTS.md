...
Dedup stops DOUBLE sends; the delivery ledger (`@ethosagent/delivery-ledger`, wired via `GatewayConfig.deliveryLedger`) stops LOST ones. Four reply paths — non-streaming final, errored/interrupted final, hook-claimed (`gateway_message`) reply, and the streaming terminal edit — write a `pending` obligation BEFORE the platform call and mark it `delivered` only when `DeliveryResult.ok === true`. "Resolved without throwing" is NOT confirmation: every adapter catches platform failures and returns `{ok:false}`, so that definition would mark the exact failures the ledger exists to catch as delivered. `Gateway.sweepPendingDeliveries()` runs after `adapter.start()` in the gateway command and redelivers whatever is still `pending`, filtered to `botKey ∈ this.bots` and claimed atomically so peer processes sharing a ledger file each deliver an obligation exactly once. Redelivery bypasses `shouldSend()` and calls `cache.record()` afterwards. Delivery is at-least-once by design, and per-adapter honesty varies — Telegram's chunked send reports `{ok:true}` on a partially delivered multi-chunk message. Slash-command acks, voice sendVoice/sendAudio, and the background-job wake notice are deliberately NOT covered.
